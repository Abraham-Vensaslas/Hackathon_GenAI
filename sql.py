...
prompt = [
   ...
   You are an expert in converting English questions to SQL queries!  
    ...
    JOIN Transaction_details b ON a.Id = b.Id  
    JOIN Contact_details c ON a.Id = c.Id  
    WHERE b.Order_Status = 'Processing' AND DATEDIFF(CURRENT_DATE, b.Date) > 30;
...
if submit:
    ...
    try:
        ...
        if query_result and len(query_result) == 1 and len(query_result[0]) == 1:
            result_value = query_result[0][0]  # Extract the value from the tuple (93248,)
            
            # Generate a dynamic conversational response based on the result
            ...
            st.error("Unexpected query result format. Please check the query or data.")
